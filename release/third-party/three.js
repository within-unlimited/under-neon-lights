	...
	"includes": [
		...
		[
			...
			[
				...
				"\t\t*/",
				"",
				"\t\t//\t'float o = translucency;',// * ( 1.0 - pow( average( layer ), 3.0 ) );',",
				...
				"\t\t'void main() {',",
				"",
				"\t\t\t'float fogFactor = clamp( fogFar / size, 0.0, 1.0 );',",
				"",
				"\t\t\t'float dist = distance( vec2( 0.5 ), vUv ) * 2.0;',",
				"\t\t\t'dist = clamp( dist, 0.0, 1.0 );',",
				"\t\t\t'float shadow = smoothstep( 0.09, 0.15, pow( dist, 0.5 ) );',",
				"",
				"\t\t\t'vec3 col = mix( color, fogColor, pow( dist, 0.75 ) );',",
				"\t\t\t'col = mix( color * 0.75, col, shadow );',",
	...
	"effects": [
		[
			...
			[
				...
				"\tclock.getDelta();",
				"\t",
				"\tif ( neon.enabled ) neon.setScene( group );",
				"",
				"}",
				"",
				"function end () {",
				...
				"}",
				"",
				"function update ( progress, timeDelta ) {",
				"",
				...
				"",
				"\tneon.motionVector.x = -amplitude * Math.cos( direction.value );",
				"    neon.motionVector.y = 0;",
				"    neon.motionVector.z = -amplitude * Math.sin( direction.value );",
				"    neon.motionVector.applyAxisAngle( new THREE.Vector3( 0, 1, 0 ), group.rotation.y );",
				"",
				"\tmaterial.uniforms.fogColor.value.set( scene.fog.color );",
		...
		[
			...
			[
				...
				"",
				"\tneon.motionVector.x = -direction * temp.x * 0.5;",
				"    neon.motionVector.y = temp.y * 0.5;",
				"    neon.motionVector.z = -direction * temp.z * 0.5;",
